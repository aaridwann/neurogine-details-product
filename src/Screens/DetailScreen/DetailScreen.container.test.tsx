...
describe('Detail screen container', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (useProductDetail as jest.Mock).mockReturnValue(mockHookReturn);
  });

  it('should call detail component with correct props', async () => {
    await render(<DetailsScreen route={mockRoute} navigation={mockNavigation} />);

    expect(DetailScreenComponent).toHaveBeenCalled();

    const passedProps = (DetailScreenComponent as unknown as jest.Mock).mock.calls[0][0];
